...
def dict_product(dicts):
    ...
    #return (dict(itertools.izip(dicts, x)) for x in itertools.product(*dicts.values()))
...
def input_directory_builder(folder_name, base_path):
    ...
    folder_name inside of base_path if base_path does not exist and if
    base_path does exist, just creates folder_name inside of it.
    """
...
def build_input_files(filename, base_path='input_files', out=sys.stdout):
    ...
    for i, value_set in enumerate(formatted_trials):
        ...
        fname = os.path.join(calling_dir,
                             base_path,
                             case_name,
                             case_name + '.fds')
    ...
    log_path_name = os.path.join(calling_dir, base_path, file_name[:-4]
                                 + '.log')
...
def input_file_paths(base_path):
    ...
    so they can be iterated over in the main body of the program via
    multiprocessing
    """
    paths = []
...
def int2base(num,b,numerals="abcdefghijklmnopqrstuvwxyz"):
    return ((num == 0) and numerals[0]) or (int2base(num // b, b, numerals).
                                            lstrip(numerals[0])
                                            + numerals[num % b])
...
def FDSa_parser(txt, filename, IOoutput=sys.stdout):
    ...
    # double check with the occurance of FDSa 'reserved' keywords
    # 'title' or 'name'
    if (len(params) % 2 != 0) and (params[0].lower() == ('title')):
        # based on the following idiom
        # https://stackoverflow.com/questions/3303213
        param_dict = OrderedDict(zip(params[1::2], params[2::2]))
        param_list = [params[0]]
        param_list.extend(params[1::2])
        param_name_dict = OrderedDict(zip(param_list, params_raw))
    else:
        param_dict = OrderedDict(zip(params[::2], params[1::2]))
        param_list = params[::2]
        param_name_dict = OrderedDict(zip(param_list, params_raw))
...
def eval_parsed_FDS(param_dict, IOoutput=sys.stdout):
    ...
    it to create python readable arrays that can be broken up for the
    parametric studies.
    """
    permutations = 1
    for key, value in param_dict.items():
        ...
        param_dict[key] = eval(
                               value_str, {"__builtins__": None},
                               {"np": np, "np.linspace": np.linspace,
                                "np.logspace": np.logspace}
                              )
        value_split = value.split(',')

        assert float(value_split[2]) >= 1, "the number of steps is not an integer: %r"\
            % float(value_split[2].strip())

        permutations *= int(value_split[2])

        IOoutput.write(key + ' varied between ' + str(value_split[0])
                       + ' and ' + str(value_split[1]) + ' in '
                       + str(value_split[2]) + ' step(s)' + '\n')
    ...
    for i, v in enumerate(trials):
        case_temp = 'case ' + str(i+1).zfill(3) + ': '
        logfile += case_temp
